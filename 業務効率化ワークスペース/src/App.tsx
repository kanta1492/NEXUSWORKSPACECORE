...
export default function App() {
  ...
  const [currentUserEmail, setCurrentUserEmail] = useState<string | null>(() => {
    const active = sessionStorage.getItem("AUTH_SESSION_ACTIVE");
    if (active === "true") {
      ...
      >
        ...
        <div className="p-6 border-b border-[#E2E8F0] flex items-center justify-between" id="sidebar-header">
          <div 
            onClick={() => {
              ...
              addSystemLog(nextState 
                ? "[管理者特権付与] ブランドアイコン押下により現在セッションに管理者特別権利が付与されました。" 
                : "[管理者特権解除] ブランドアイコン押下により管理者権利が解除されました。"
              );
            }}
            className="text-lg font-extrabold tracking-tight flex items-center gap-2.5 text-slate-800 cursor-pointer hover:opacity-80 active:scale-95 duration-100 transform select-none"
            title="クリックしてアカウント権限の切り替え"
          >
        ...
        <nav className="flex-1 p-5 space-y-1.5 overflow-y-auto" id="sidebar-navi">
          ...
          >
            ...
            {calendarEvents.filter(e => !e.notified && e.date === "2026-06-01").length > 0 && (
              <span className="ml-auto w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-500 border border-amber-500/20 relative z-10">
                {calendarEvents.filter(e => !e.notified && e.date === "2026-06-01").length}
              </span>
            )}
          </button>
