...
function getGeminiClient(): GoogleGenAI {
  if (!aiClient) {
    ...
    if (!apiKey) {
      throw new Error("GEMINI_API_KEY environment variable is required to perform AI summarization.");
    }
...
// 4. Summarize Minutes / Generate Automatic Minutes (議事録自動作成)
app.post("/api/gemini/summarize-minutes", handleAsync(async (req, res) => {
  try {
    const { transcript, title, option } = req.body;
    if (!transcript || typeof transcript !== "string" || transcript.trim().length === 0) {
      return res.status(400).json({ error: "トランスクリプト（発言記録）が空です。音声を録音するか、テキストを編集してください。" });
    }

    const ai = getGeminiClient();
    const systemPrompt = `
あなたはプロフェッショナルなIT事務・プロジェクトマネージャーです。
対面やオンラインミーティングで録音された雑多な音声文字起こし(トランスクリプト)を分析し、
非常に洗練された、整理された「議事録 (Meeting Minutes)」を日本語で作成してください。

作成する議事録には以下の構成を含めてください：
1. **会議概要** (日時・議題・参加者などについて、記述内容から推測または自動提示して記載。会議名: ${title || "無題の会議"})
2. **決定事項 (Decisions)** (箇条書きで、会議中何が決定・合意されたのかを明確に提示。)
3. **ToDoリスト・Next Actions** (誰が・何を・いつまでにやるべきか。担当者や期日の記述があれば明記し、無ければ必要な行動項目を展開。)
4. **議論の要約 (Detailed Summary)** (主要なトピック、出された意見、懸念点などを整理して読みやすく要約。)
5. **AIコメント・気づき** (ミーティングを推進するためのアドバイス、今後のタスク管理や段取りに関する簡単なアドバイス。)

出力フォーマットはすべて読みやすく構造化されたMarkdown形式にしてください。
`;

    const userPrompt = `
以下はミーティングの文字起こしデータです。
これを元に会議の決定事項やToDo、要約を含むプロフェッショナルな議事録を編集・作成してください。

【追加のオプション・個別指示】:
${option || "特記なし。全体的な論点整理と Next Actions の明確化をお願いします。"}
...
`;
    ...
    console.error("Gemini minutes summarization error:", error);
    res.status(500).json({ error: error.message || "議事録のAI要約・作成に失敗しました。" });
  }
}));
