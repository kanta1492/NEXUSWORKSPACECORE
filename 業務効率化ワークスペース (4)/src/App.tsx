...
import MinutesTracker from "./components/MinutesTracker";
...
export default function App() {
  ...
  const [currentUserDisplayName, setCurrentUserDisplayName] = useState<string>(() => {
    ...
    if (activeUser) {
      ...
      if (mapStr) {
        try {
          const userMap = JSON.parse(mapStr);
          if (userMap[activeUser.toLowerCase()]?.displayName) {
            return userMap[activeUser.toLowerCase()].displayName;
          }
        } catch (e) {}
      }
      return activeUser.split("@")[0];
    }
    return "ゲスト";
  });
  ...
  const handleUpdateCredentials = async (
    ...
    // De-serialize BIZ_USERS_MAP to maintain multi-user settings sync
    const mapStr = localStorage.getItem("BIZ_USERS_MAP") || "{}";
    let userMap: Record<string, any> = {};
    try {
      userMap = JSON.parse(mapStr);
    } catch (e) {}

    const oldEmailLower = currentUserEmail ? currentUserEmail.toLowerCase() : "";
    const newEmailLower = newEmail.trim().toLowerCase();

    // Fetch or construct user credentials
    const userProfile = userMap[oldEmailLower] || {
      registeredAt: creds.registeredAt || new Date().toISOString()
    };

    // Update with new values
    const finalDisplayName = newDisplayName !== undefined ? newDisplayName.trim() : (userProfile.displayName || newEmail.trim().split("@")[0]);
    ...
    // Delete old key if email changed
    if (oldEmailLower && oldEmailLower !== newEmailLower) {
      delete userMap[oldEmailLower];
    }
    userMap[newEmailLower] = updatedUserEntry;
    localStorage.setItem("BIZ_USERS_MAP", JSON.stringify(userMap));

    // Also update face credentials list if existing
    try {
      const listStr = localStorage.getItem("FACE_AUTH_CREDS_LIST");
      if (listStr) {
        const faceAuthList = JSON.parse(listStr);
        let updatedList = faceAuthList.filter((x: any) => x.email.toLowerCase() !== oldEmailLower);
        updatedList.push({ email: newEmail.trim(), password: newPasswordPlain });
        localStorage.setItem("FACE_AUTH_CREDS_LIST", JSON.stringify(updatedList));
      }
    } catch (e) {}

    // Update active user in state & localStorage
    localStorage.setItem("AUTH_CURRENT_USER", newEmail.trim());
    setCurrentUserEmail(newEmail.trim());
    setCurrentUserDisplayName(finalDisplayName);

    const log = createSecurityLogEntry(
      `アカウント認証情報・氏名が更新されました（ユーザーID: ${creds.email} -> ${newEmail.trim()}, 表示名: ${finalDisplayName}）`,
      "profile_change",
      "success"
    );
    setSecurityLogs(prev => [log, ...prev]);

    return { success: true, message: "アカウント設定および氏名情報が安全に適用されました。" };
  };

  const handleDeleteAccount = () => {
    const activeUser = currentUserEmail;
    if (!activeUser) return;

    // Create deletion security audit log
    const log = createSecurityLogEntry(
      `退会処理: メンバーアカウント「${activeUser}」が完全に消去されました。`,
      "profile_change",
      "danger"
    );
    setSecurityLogs(prev => [log, ...prev]);

    // Remove user profile from maps
    const mapStr = localStorage.getItem("BIZ_USERS_MAP");
    if (mapStr) {
      try {
        const userMap = JSON.parse(mapStr);
        delete userMap[activeUser.toLowerCase()];
        localStorage.setItem("BIZ_USERS_MAP", JSON.stringify(userMap));
      } catch (e) {}
    }

    // Remove face registration biometrics from list
    try {
      const listStr = localStorage.getItem("FACE_AUTH_CREDS_LIST");
      if (listStr) {
        const faceList = JSON.parse(listStr);
        const updatedFaceList = faceList.filter((x: any) => x.email.toLowerCase() !== activeUser.toLowerCase());
        localStorage.setItem("FACE_AUTH_CREDS_LIST", JSON.stringify(updatedFaceList));
      }
    } catch (e) {}

    // Flush current active session variables
    localStorage.removeItem("AUTH_SESSION_ACTIVE");
    localStorage.removeItem("AUTH_CURRENT_USER");
    sessionStorage.removeItem("AUTH_SESSION_ACTIVE");

    // Revert state hook variables
    setCurrentUserEmail(null);
    setIsAdmin(false);
    setCurrentUserDisplayName("ゲスト");

    addSystemLog(`Security System: アカウント「${activeUser}」の退会処理が正常に完了しました。`);
  };
  ...
  useEffect(() => {
    const checkInterval = setInterval(() => {
      ...
      calendarEvents.forEach(event => {
        // Evaluate condition: Event date is today, and event is not yet notified, and belongs to current user
        const isEventForCurrentUser = !event.userEmail || (currentUserEmail && event.userEmail.toLowerCase() === currentUserEmail.toLowerCase());
        if (!event.notified && event.date === todayStr && isEventForCurrentUser) {
    ...
    return (
      <AuthGate
        onLoginSuccess={(email) => {
          ...
          const mapStr = localStorage.getItem("BIZ_USERS_MAP");
          let dName = email.split("@")[0];
          if (mapStr) {
            try {
              const userMap = JSON.parse(mapStr);
              if (userMap[email.toLowerCase()]?.displayName) {
                dName = userMap[email.toLowerCase()].displayName;
              }
            } catch (e) {}
          }
          setCurrentUserDisplayName(dName);
        }}
      ...
      >
        ...
        <nav className="flex-1 p-5 space-y-1.5 overflow-y-auto" id="sidebar-navi">
          ...
          >
            ...
            <Sparkles className="w-4 h-4 text-[#06B6D4] relative z-10 animate-pulse" />
            <span className="relative z-10 font-bold text-slate-850">AI 議事録自動作成</span>
          </button>
              ...
              {activeTab === "minutes" && (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-black text-slate-800">C# / Gemini Meeting Minutes Compiler (議事録自動作成)</h2>
                  </div>
                  <MinutesTracker
