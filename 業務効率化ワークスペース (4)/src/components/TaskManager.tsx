...
export default function TaskManager({
  ...
  // Parse BIZ_USERS_MAP for visual names mapping
  ...
  // Get all unique users from BIZ_USERS_MAP
  ...
  // Actual filtering based on isAdmin & currentUserEmail
  const filteredTasks = React.useMemo(() => {
    if (!isAdmin) {
      return tasks.filter(t => !t.userEmail || t.userEmail.toLowerCase() === currentUserEmail?.toLowerCase());
    } else {
      if (selectedUserFilter === "all") {
        return tasks;
      }
      return tasks.filter(t => t.userEmail?.toLowerCase() === selectedUserFilter.toLowerCase());
    }
  }, [tasks, isAdmin, currentUserEmail, selectedUserFilter]);

  const getUserNameByEmail = (email?: string) => {
    if (!email) return "共通/システム";
  ...
  return (
    <div className="space-y-6" id="task-manager-root">
      ...
      <div className={isAdmin ? "grid grid-cols-1 gap-6" : "grid grid-cols-1 lg:grid-cols-3 gap-6"}>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 flex flex-col justify-between shadow-sm" id="view-controls-panel">
          <div>
            ...
            {isAdmin && (
              <div className="mb-4 bg-cyan-50/50 border border-cyan-100 p-3.5 rounded-xl space-y-1.5" id="admin-member-filter">
                <label className="block text-[10px] text-cyan-600 font-bold">メンバー別に絞り込み（管理者限定）</label>
                <select
                  value={selectedUserFilter}
                  onChange={(e) => setSelectedUserFilter(e.target.value)}
                  className="w-full bg-white border border-cyan-200 focus:outline-none focus:border-[#06B6D4] focus:ring-1 focus:ring-[#06B6D4]/30 rounded-xl px-2.5 py-1.5 text-xs text-slate-800 font-bold cursor-pointer"
                >
                  <option value="all">👥 すべてのメンバー（全件表示）</option>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[#E2E8F0] bg-slate-50 text-slate-500 text-[10px] uppercase font-bold tracking-wider">
                ...
                {isAdmin && <th className="py-4 px-6 w-28">担当者</th>}
                <th className="py-4 px-6 w-28">優先度</th>
                <th className="py-4 px-6 w-36">{isAdmin ? "ステータス" : "ステータス変更"}</th>
                <th className="py-4 px-6 w-32">時間効率 (実績/予定)</th>
                <th className="py-4 px-6 w-32 font-mono">期限日</th>
                <th className="py-4 px-6 w-16 text-center">{isAdmin ? "権限" : "操作"}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#E2E8F0] text-slate-700">
              {filteredTasks.length === 0 ? (
                <tr>
                  <td colSpan={isAdmin ? 7 : 6} className="py-12 text-center text-slate-400 font-semibold text-xs">
                  ...
                  <tr key={task.id} className="hover:bg-slate-50/50 transition duration-100">
                    ...
                    {isAdmin && (
                      <td className="py-4 px-6 text-xs font-bold text-slate-600">
                        {getUserNameByEmail(task.userEmail)}
                      </td>
                    )}
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6">
                      {isAdmin ? (
                        <span className="text-xs font-bold text-slate-600 bg-slate-100 border border-slate-200 px-2 py-1 rounded-lg">
                          {task.status === "todo" ? "未着手" : task.status === "doing" ? "着手中" : "完了"}
                        </span>
                      ) : (
                        <select
                          value={task.status}
                    ...
                    <td className="py-4 px-6 text-center">
                      {isAdmin ? (
                        <span className="text-[10px] text-slate-400 font-bold bg-slate-50 border border-slate-150 px-1.5 py-0.5 rounded cursor-default">
                          閲覧のみ
                        </span>
                      ) : (
                        <button
            ...
            tasks.map((task) => (
              ...
              >
                <div>
                  ...
                  {task.userEmail && (
                    <div className="text-[10px] bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-lg px-2 py-0.5 font-bold inline-block max-w-full truncate mb-2">
                      👤 {getUserNameByEmail(task.userEmail)}
                    </div>
                  )}
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-slate-200 max-h-24 overflow-y-auto">
                          {task.checklist.map((item) => (
                            <button
                              ...
                              className={`w-full text-left flex items-start gap-1.5 text-[10px] text-slate-600 ${isAdmin ? "cursor-default" : "hover:text-slate-800 transition"}`}
                            >
                  ...
                  {task.checklist.length === 0 && activeChecklistForId === task.id && !isAdmin && (
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <div className="flex gap-1 text-[10px] h-5">
                    {isAdmin ? (
                      <span className="text-[10px] text-slate-400 bg-slate-50 border border-slate-150 px-1.5 py-0.5 rounded cursor-default font-bold">
                        閲覧のみ
                      </span>
                    ) : (
                      <>
