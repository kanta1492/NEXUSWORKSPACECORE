...
interface SettingsAndLogsProps {
  ...
  onUpdateCredentials: (oldPasswordPlain: string, newEmail: string, newPasswordPlain: string, newDisplayName?: string) => Promise<{ success: boolean; message: string }>;
...
export default function SettingsAndLogs({
  ...
  // Administrative member filter
  const [selectedUserFilter, setSelectedUserFilter] = useState<string>("all");

  // Resolve User Maps from local storage for admin filters
  ...
  // Extract unique users who performed actions in logs
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (parentResult.success) {
        ...
        setNewPassword("");
        setConfirmPassword("");
        addSystemLog(`Security System: アカウント情報・氏名が更新されました。 ID: ${newEmail}, 氏名: ${newDisplayName.trim()}`);
      } else {
  ...
  const filteredLogs = securityLogs.filter(log => {
    const logUser = log.userEmail ? log.userEmail.toLowerCase() : "";
    const activeUser = email ? email.toLowerCase() : "";
    
    // Non-admin can only see logs they created or system logs with no explicit user scopes
    if (!isAdmin && log.userEmail && logUser !== activeUser) {
      return false;
    }

    // Admin member specific filter
    if (isAdmin && selectedUserFilter !== "all") {
      if (selectedUserFilter === "system_only") {
        if (log.userEmail) return false;
      } else {
        if (!log.userEmail || log.userEmail.toLowerCase() !== selectedUserFilter.toLowerCase()) {
          return false;
        }
      }
    }
          ...
          >
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm lg:col-span-2 space-y-6">
              ...
              <form onSubmit={handleUpdate} className="space-y-4">
                ...
                {/* 2.5. New Display Name */}
                <div className="space-y-1.5" id="settings-display-name-container">
                  ...
                  <input
                    ...
                    placeholder="例：佐藤 健一"
                    className="w-full bg-white border border-[#E2E8F0] focus:border-[#06B6D4] focus:ring-1 focus:ring-[#06B6D4]/30 focus:outline-none rounded-xl px-4 py-2 text-xs text-slate-800 font-sans font-bold"
                  />
                  <p className="text-[10px] text-slate-400">会議の議事録、ガントチャート、タスク管理者割り当てなどで発言者または担当者として表示されます。</p>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-rose-50/45 border border-rose-100 rounded-2xl p-5 text-center space-y-3">
                <span className="text-[10px] text-rose-600 font-extrabold uppercase tracking-wider block">アカウント設定</span>
                <p className="text-[10.5px] text-slate-500 leading-normal font-medium">
                  現在ログイン中のアカウント情報をシステムデータベースから完全に消去し、即時にログアウトします。この操作は元に戻せません。
                </p>
                
                {deleteConfirm ? (
                  <div className="space-y-2 animate-fadeIn">
                    <p className="text-[10px] text-rose-600 font-bold bg-rose-100/55 p-2 rounded-xl">
                      【重要】本当に抹消しますか？メンバー内の関連情報、顔認識データ等すべて削除されます。
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="bg-rose-600 hover:bg-rose-700 text-white text-[11px] py-1.5 px-1 rounded-xl font-bold transition flex items-center justify-center gap-1 shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="bg-slate-200 hover:bg-slate-300 text-slate-700 text-[11px] py-1.5 px-1 rounded-xl font-bold transition flex items-center justify-center"
                      >
                        キャンセル
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full bg-rose-600 hover:bg-rose-700 text-white text-xs py-2 px-3 rounded-xl font-bold transition flex items-center justify-center gap-1.5 shadow-sm shadow-rose-600/10"
                  >
                    <Trash2 className="w-4 h-4" />
                    退会・アカウントの完全削除
                  </button>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  各アカウントに発生した最新のログイン成功、失敗、IPアドレス、パスワードの改定など全ての監視セキュリティ監査トレールを表示します。
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 disabled:opacity-40"
                >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {isAdmin && (
                <select
                  ...
                  className="bg-cyan-50 border border-cyan-200 focus:outline-[#06B6D4] focus:ring-1 focus:ring-[#06B6D4]/30 rounded-xl px-3 py-2 text-xs text-slate-800 font-bold cursor-pointer"
                >
                  <option value="all">👥 すべてのメンバーの監査ログ</option>
                  <option value="system_only">⚙️ システム自動トリガーのみ</option>
              ...
              <select
                ...
                className="bg-slate-50 border border-[#E2E8F0] rounded-xl px-3 py-2 text-xs text-slate-700 outline-none focus:border-[#06B6D4]"
              >
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody>
                    {filteredLogs.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="p-8 text-center text-slate-400 bg-white leading-normal">
                        ...
                        return (
                          <tr key={log.id} className="border-t border-slate-160 hover:bg-slate-100/50 transition duration-150 bg-white">
                            ...
                            <td className="p-3 text-slate-600 font-medium text-[11px]">
                              {log.userEmail ? (
                                <span className="font-mono text-[10.5px] bg-slate-105 text-slate-700 px-2 py-0.5 rounded-md border border-slate-200">
                                  {log.userEmail}
                                </span>
                              ) : (
