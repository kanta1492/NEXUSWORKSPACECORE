...
export default function CalendarView({
  ...
  // Resolve User Maps from local storage for admin filters and display name mappings
  ...
  // Extract unique users who have registered events standard in BIZ_USERS_MAP
  ...
  // Administrative member filter state for Calendar
  const [selectedUserFilter, setSelectedUserFilter] = useState<string>("all");

  // Actual event filtering based on isAdmin and current user email
  const filteredEvents = React.useMemo(() => {
    if (!isAdmin) {
      return events.filter(ev => !ev.userEmail || ev.userEmail.toLowerCase() === currentUserEmail?.toLowerCase());
    } else {
      if (selectedUserFilter === "all") {
        return events;
      }
      return events.filter(ev => ev.userEmail?.toLowerCase() === selectedUserFilter.toLowerCase());
    }
  }, [events, isAdmin, currentUserEmail, selectedUserFilter]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm flex flex-col justify-between" id="calendar-grid-card">
                      ...
                      <div className="space-y-0.5 mt-1.5 max-h-[36px] overflow-hidden">
                        {cellEvents.slice(0, 2).map((ev) => (
                          ...
                          >
                            ...
                            {ev.userEmail && isAdmin && (
                              <span className="font-bold text-[#06B6D4] text-[7.5px] mr-0.5">[{getUserNameByEmail(ev.userEmail).slice(0, 4)}]</span>
                            )}
                            {ev.title}
                          </div>
                        ))}
          ...
          <div className="text-[10px] text-slate-400 font-mono mt-6 border-t border-slate-100 pt-3 flex items-center justify-between">
            <span>カレンダースケジューラ配列: {filteredEvents.length}件表示中 (全{events.length}件)</span>
        ...
        <div className="flex flex-col gap-6" id="calendar-sidebar-details">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm flex-1 flex flex-col justify-between" id="schedule-list-panel">
            <div>
              ...
              {isAdmin && (
                <div className="mb-4 bg-cyan-50/50 border border-cyan-100 p-3 rounded-xl space-y-1.5" id="admin-calendar-filter">
                ...
                <AnimatePresence mode="popLayout">
                  {selectedDayEvents.length === 0 ? (
                    ...
                    selectedDayEvents.map((ev) => {
                      const isOwnEvent = !isAdmin && (!ev.userEmail || (currentUserEmail && ev.userEmail.toLowerCase() === currentUserEmail.toLowerCase()));
                      return (
                        <motion.div
                          layout
                          key={ev.id}
                        ...
                        >
                          <div className="space-y-1 overflow-hidden flex-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {/* User display name */}
                              {ev.userEmail && (
                                <span className="text-[8.5px] bg-[#06B6D4]/10 text-[#06B6D4] px-1.5 py-0.5 rounded font-extrabold shadow-sm flex items-center gap-0.5">
                                  👤 {getUserNameByEmail(ev.userEmail)}
                                </span>
                              )}
                            </div>
                          ...
                          {isOwnEvent ? (
                            ...
                            <span 
                              className="text-[9px] text-slate-400 bg-slate-100 border border-slate-200 px-2 py-1 rounded-lg shrink-0 font-bold self-center flex items-center gap-0.5 cursor-default"
                              title="ほかのユーザーの予定のため削除できません（閲覧のみ）"
                            >
