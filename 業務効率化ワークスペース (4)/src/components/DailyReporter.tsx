...
export default function DailyReporter({
  ...
  // Administrative user filter for DailyReporter
  ...
  // Extract members list for filter
  ...
  // Filtering based on role and active selection
  const filteredWorkLogs = React.useMemo(() => {
    if (!isAdmin) {
      return workLogs.filter(w => !w.userEmail || w.userEmail.toLowerCase() === currentUserEmail?.toLowerCase());
    } else {
      if (selectedUserFilter === "all") {
        return workLogs;
      }
      return workLogs.filter(w => w.userEmail?.toLowerCase() === selectedUserFilter.toLowerCase());
    }
  }, [workLogs, isAdmin, currentUserEmail, selectedUserFilter]);
  ...
  const handleGenerateReportByAI = async () => {
    ...
    const todayTasks = filteredTasks.map((t) => ({
      title: t.title,
    ...
    const todayTimeline = filteredWorkLogs.map((l) => ({
      time: l.time,
      task: l.title,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8" id="daily-reporter-root">
      ...
      <div className="lg:col-span-5 space-y-6">
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm" id="chronological-activity-list">
          ...
          {isAdmin && (
            <div className="mb-4 bg-cyan-50/50 border border-cyan-100 p-3 rounded-xl space-y-1.5" id="admin-reporter-filter">
          ...
          <div className="flex items-center justify-between mb-4 border-b border-[#E2E8F0] pb-2">
            ...
            <span className="bg-slate-50 border border-[#E2E8F0] text-slate-600 font-mono text-[10px] py-0.5 px-2.5 rounded-full font-bold">
              タイムライン累計: {filteredWorkLogs.reduce((acc, l) => acc + l.durationMinutes, 0)} 分
            </span>
          </div>
                ...
                {filteredWorkLogs
                  ...
                  .map((log) => (
                    ...
                    >
                      ...
                      {log.userEmail && (
                        <div className="text-[10px] text-slate-500 font-bold mt-1 shrink-0 flex items-center gap-0.5">
                          👤 投稿者: {getUserNameByEmail(log.userEmail)}
                        </div>
                      )}
